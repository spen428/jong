...
public final class GraphicsConstants {
	...
	public static final float WALL_TILES_Y_OFFSET_MM = TILE_WIDTH_MM * GameConstants.WALL_WIDTH_TILES * 0.60f;
	public static final float HAND_TILES_Y_OFFSET_MM = (PLAYING_SURFACE_RADIUS_MM - WALL_TILES_Y_OFFSET_MM) * 1.70f;
	public static final float DISCARD_TILES_Y_OFFSET_MM = TILE_WIDTH_MM * GameConstants.DISCARD_WIDTH_TILES * 0.80f;
	...
	private static final Material mat = new Material(ColorAttribute.createDiffuse(Color.WHITE));
	private static final int attr = Usage.Position | Usage.Normal | Usage.TextureCoordinates;

	public static final Model MODEL_TILE_OLD = mb.createBox(TILE_WIDTH_MM, TILE_HEIGHT_MM, TILE_THICKNESS_MM, mat,
			attr);

	public static final Model MODEL_RIICHI_STICK = mb.createBox(RIICHI_WIDTH_MM, RIICHI_THICKNESS_MM, RIICHI_HEIGHT_MM,
			mat, attr);

	/**
	 * Model of a Mahjong Tile, orientated face-down.
	 */
	public static final Model MODEL_TILE;
	...
	static {
		...
		mb.begin();
		mb.part("back", GL20.GL_TRIANGLES, attr, new Material(ColorAttribute.createDiffuse(Color.ORANGE))).box(
				halfTileHeight, halfTileWidth, (faceThick / 2) + frontThick + (backThick / 2), halfTileHeight * 2,
				halfTileWidth * 2, backThick);
		mb.part("front", GL20.GL_TRIANGLES, attr, new Material(ColorAttribute.createDiffuse(Color.WHITE))).box(
				halfTileHeight, halfTileWidth, (faceThick / 2) + (frontThick / 2), halfTileHeight * 2,
				halfTileWidth * 2, frontThick);
		mb.part("face", GL20.GL_TRIANGLES, attr, new Material(ColorAttribute.createDiffuse(Color.WHITE)))
				.box(halfTileHeight, halfTileWidth, 0, halfTileHeight * 2, halfTileWidth * 2, faceThick);
		MODEL_TILE = mb.end();
	}
